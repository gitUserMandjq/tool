...
public class StringUtils {
    ...
    public static String format(String defaultMsg, Object... arg) {
        return MessageFormat.format(defaultMsg.replaceAll("'", "''"), arg);
    }
...
//    }
    ...
    //处理引用字段
    public static String cast(String option) {
        String jsonString="[{].*\"key\":.*[}]";
        Pattern jsonPattern=Pattern.compile(jsonString);
        if(!jsonPattern.matcher(option).matches()) {
            if (option.indexOf(".") > 0) {
                // 去掉后面无用的零
                option = option.replaceAll("0+?$", "");
                // 如小数点后面全是零则去掉小数点
    ...
    public static String castNum(String option) {
        if (option.indexOf(".") > 0) {
            // 去掉后面无用的零
            option = option.replaceAll("0+?$", "");
            // 如小数点后面全是零则去掉小数点
